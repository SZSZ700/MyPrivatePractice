...
//במסך המשתמש מזין משקל וגובה, וכאשר לוחץ על כפתור החישוב המערכת בודקת
//שכל השדות מולאו ושהקלט תקין. לאחר מכן מתבצע חישוב מדד מסת הגוף והתוצאה
//מוצגת למשתמש מיידית. במקביל נשלחת בקשה לצד השרת לשמירת הערך עבור
//המשתמש המחובר. אם השמירה הצליחה, מוצגת הודעת הצלחה והתוצאה נשמרת גם
//בזיכרון המקומי כגיבוי. אם השמירה נכשלה, מוצגת הודעת שגיאה והמשתמש יכול
//לנסות שוב.
//בנוסף למסך זה יש רכיב של מעקב קלוריות יומי: המשתמש יכול להוסיף כמות, להפחית
//כמות או לאפס את הערך. בכל פעולה המערכת בודקת שהקלט תקין, מעדכנת את
//הסכום היומי ומציגה סטטוס ביחס ליעד היומי. לאחר כל שינוי נשלחת בקשה לשרת לשמירת הסכום,
//ובמקרה הצלחה נשמר גם גיבוי בזיכרון המקומי
// -------------------------------------------------------------
